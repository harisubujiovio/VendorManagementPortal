...
import { ContracttypeService } from 'src/app/services/contracttype.service';
@Component({
  selector: 'app-contracttype-list',
  templateUrl: './contracttype-list.component.html',
  styleUrls: ['./contracttype-list.component.css']
})
export class ContracttypeListComponent implements OnInit, AfterViewInit {
  ...
  constructor(private contracttypeS: ContracttypeService, private router: Router, private dialog: MatDialog,
    private errorService: ErrorhandlerService) { }

  ngOnInit(): void {
    this.dataSource = new ResourceDataSource(this.contracttypeS);
  ...
  public redirectToDetails = (id: string) => {
    let url: string = `/contracttype/details/${id}`;
    this.router.navigate([url]);
  }
  public redirectToUpdate = (id: string) => {
    let url: string = `/contracttype/edit/${id}`;
    this.router.navigate([url]);
  }
  public delete = (id: string) => {
    ...
    dialogRef.afterClosed().subscribe((confirmed: boolean) => {
      if (confirmed) {
        this.contracttypeS.delete(id).subscribe({
          next: () => {
